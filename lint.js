...
function checkForImproperlyIndentedFencedCodeBlocks(fileName, text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (codeBlockMatch) {
      ...
      if (
        startingColumn === 0 ||
        startingColumn === getCorrectStartingColumnForLine(lines, i)
      ) {
        continue;
      }
      ...
      console.log(
        fileName +
          ...
          i +
          1 +
          ": A fenced code block following a list item must be indented to the first non-whitespace character of the list item."
      );
    }
  }
